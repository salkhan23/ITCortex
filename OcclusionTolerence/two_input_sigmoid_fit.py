...
Here diagnostic group variances refers to the variance explained by the grouping according to
diagnosticity.
one group is diagnostic and the other group is nondiagnostic
...
def main(visibilities, fire_rates, d_to_t_var_ratio):
    ...
    ax.set_xlabel("Visibility Combined", fontsize=font_size)
    ax.set_ylabel("Normalized fire rate (spikes/s)", fontsize=font_size)
    ax.set_title("Tuning curve along axis\n diagnostic visibility=nondiagnostic visibility",
                 fontsize=font_size + 10)

    ax.legend(fontsize=font_size, loc=4)

    ax.annotate('w_c=%0.2f, bias=%0.2f' % (w_combined, bias),
                xy=(0.40, 0.95),
